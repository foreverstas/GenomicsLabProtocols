...
{{% /notice %}}
{{% notice "info" %}}
Продолжительность фрагментации определяется заранее в предварительном эксперименте. По опыту это где-то 16-20 циклов.
{{% /notice %}}
...
Для пробоподготовки требуется не менее 10 нг ДНК. В идеале, чем больше - тем лучше. Нам удавалось получать 300 нг на одну иммунопреципитацию. Однако нужно понимать, что большой выход может быть результатом сильного неспецифичного связывания антител. Исключить такую возможность можно с помощью qPCR с несколькими отрицательными контролями.

-   *Какой выход ожидается в AB-?*

В идеальном эксперименте в образце AB- должно выделяться минимальное количество ДНК (не более 10-20 пг, в 1000-100000 раз меньше, чем в AB+). Это говорит о том, что магнитные частицы и пластик пробирки не взаимодействуют с хроматином. Если же в ходе эксперимента хроматин осаждается на шарах, то перед иммунопреципитацией необходимо провести дополнительную очистку хроматина шариками без антител. Для этого отмытые шарики (без антител) добавляют к фрагментированному хроматину на 1-3 часа, после чего шарики удаляются и ставится иммунопреципитация.

-   *Какой выход ожидается в Input?*

Чтобы понять это, перед началом эксперимента необходимо оценить стартовое количество ДНК. Например, путем предварительного эксперимента было посчитано, что из 15 мкл 12-часовых эмбрионов можно выделить 12 мкг геномной ДНК. Значит, если на эксперимент взяли 50 мкл эмбрионов, то на старте будет 40 мкг ДНК. Допустим, во время отмывок и фиксаций потери объема составили 10%. Значит, после фрагментации должно остаться около 36 мкг ДНК. Если весь хроматин разделили на AB+ и AB- поровну, а на Input взяли 10% от объема AB+, то в Input стоит ожидать \~1.5 мкг ДНК. Обычно, поскольку эффективность экстракции ядер во время отмывок ниже 100%, в ходе фрагментирования часть хроматина остается в осадке, а также из-за потерь во время преципитации нуклеиновых кислот выход ДНК в Input несколько ниже, так что высадить 50% от этого количества будет совершенно нормальным. Если же выход ниже 10%, то причиной может быть недофиксация, загрязнение нуклеазами и большие потери на каком-то из этапов работы. Все это может приводить говорить о неудачном эксперименте ChIP.

-   *У меня маленький выход ДНК в AB+, что мне делать?*

Выход ДНК зависит главным образом от трех факторов: содержания белка в хроматине, количества стартового материала на момент добавления антител и количества самих антител.

Как правило, при изучении транскрипционных факторов, на одну иммунопреципитацию берут не мене 50 мкг ДНК (фиксированного хроматина). Для гистонов или белков, обладающих широким распространением по геному, количество стартового материала в 7-10 раз ниже. Чтобы определить, как много ДНК можно высадить в ходе иммунопреципитации, достаточно оценить, сколько сайтов связывания в геноме содержит данный ТФ, или сколько процентов генома занимает данная гистоновая модификация. Эту информацию можно подчерпнуть из опубликованных экспериментов ChIP-seq, либо из результатов иммуноокрашивания.

Количество антител для иммунопреципитации определяется их качеством. Хорошие антитела обладают отличной емкостью (сотни нанограмм ДНК на мкг антител). В большинстве случаях выход сильно ниже. Если суммарная емкость антител превышает количество метки в образце, то это приводит к увеличению неспецифичных сигналов в итоговом профиле ChIP-seq. Желательно провести предварительный эксперимент для определения правильного соотношения антител к хроматину.
