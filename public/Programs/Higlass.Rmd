...
blogdown::shortcode_open(.name = "expand", "Что важно знать про Higlass?")
```
...
```{r echo=FALSE}
blogdown::shortcode("notice","warning" ,.content = "Установка пакетов Higlass для Python и Jupyter возможна только на системе Linux. Даже в Anaconda. Даже если очень захотеть. Для пользователей Windows Higlass доступен только через интерфейс docker или путем соединения с Higlass-сервером (по URL-ссылке или через JavaScript API).")
```

```{r echo=FALSE}

blogdown::shortcode_close(.name = "expand", "Свернуть")
```
...
```{r echo=FALSE}
blogdown::shortcode_open("expand","Документация")
```
...
```{r echo=FALSE}
blogdown::shortcode_open("expand","Установка на Windows при помощи Docker")
```
...
```{r echo=FALSE}
blogdown::shortcode_open("expand","Интерфейс и работа в геномном браузере:")
```
...
```{r echo=FALSE}
blogdown::shortcode_open("expand","Интеграция с Jupyter Notebook (только для Linux)")
```
